...
public class PhysicsPassingCalc implements PassingCalc {
	...
	private static final LoggedTunable<Distance> distanceOffset = LoggedTunable.from("Shooting/Passing/Distance Offset", Inches::of, 0.0);

	public PhysicsPassingCalc() {
		// Preload polynomials
		ShooterConstants.passingFlywheelPolynomial.evaluate(0.0, 0.0);
		ShooterConstants.passingHoodPolynomial.evaluate(0.0, 0.0);
	}
	...
	public void calculate(Pose2d robotPose, ChassisSpeeds fieldSpeeds, Translation3d aimPoint) {
		...
		final var radiusMeters = shooterHubSpaceCartesian.getNorm() + PhysicsPassingCalc.distanceOffset.get().in(Meters);
		...
		final var hoodAngleRads = ShooterConstants.passingHoodPolynomial.evaluate(radiusMeters, radialVelocityMPS);
		// Logger.recordOutput("DEBUG/PhysicsShooting/Before Hood Angle", hoodAngleRads);
		final var flywheelSpeedMPS = ShooterConstants.passingFlywheelPolynomial.evaluate(radiusMeters, radialVelocityMPS);
		...
		Logger.recordOutput("Subsystems/Shooter/Passing/Effective Distance", radiusMeters, Meters);
	}
